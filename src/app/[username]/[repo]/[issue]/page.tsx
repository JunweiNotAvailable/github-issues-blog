...
const Post = () => {
  ...
  const [tempSelectedLabels, setTempSelectedLabels] = useState<any[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setSelectedLabels(issueData.labels);
  ...
  // check if post is authenticated user
  useEffect(() => {
    if (authUser && authUser?.login === owner?.login) {
      setIsMyPost(true);
    }
  }, [authUser, owner]);
